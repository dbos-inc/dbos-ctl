...
- [x] **C3. Error mapping** — `problem+json` → message, 401 → "run `dbos login`", 403 + `X-DBOS-Error: past_limit` → the upgrade message; plus the exit-code contract (auth→3, not-found→4, usage→2, else 1) via an `exitError` + `SetFlagErrorFunc`. A Teams-only 403 needs no special case — conductor sends no header, and its problem+json detail already says so. (The pre-request OAuth-gate check waits for F4, where the OAuth-only commands it guards first appear.)
- [x] **C4. Identity (whoami)** — `dbos whoami` via the generated `getCurrentUser`, with a single `selfhosted`-mode guard (static `local`). The natural end-to-end confirmation that login worked; `register` ships with the OAuth-gated org/invite flow in F4.
...
- [ ] **F3. Alerts** — `alert list|create|delete`. (`app metrics` moved up to D2, its `csv` format is F1; `schedule backfill` moved up to E4.)
