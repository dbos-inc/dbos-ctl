...
**Open question — harness mode (resolve in the D4 spike).** The executor connects with a `dbos_` key, so this likely rides the **OAuth-mode** harness (`conductortest.StartOAuth`), where keys are real, rather than the no-auth A5 harness — confirm the websocket token path accepts a `local`-org key before committing D4's populated-read tests to a mode. This fixture is **shared with E1** (workflow reads need executed workflows from exactly this running app), where a live executor is mandatory rather than merely enriching.
...
- [ ] **D4. Executor fixture + populated app reads** — stand up the conductor-connected **Go Transact app** (see **Executor fixture** in Testing) and use it to fill in the live-executor reads deferred from D2: `app executors` with a real row, `app versions` (dispatches to a live Healthy executor — a bare app returns `ErrNoHealthyExecutors`, *not* an empty list), and `app metrics` (needs an executor that ran workflows + an elapsed collection period). The app connects with a **D3-minted `api-key`** as `ConductorAPIKey`. **Token permission for the executor:** an executor connection requires the `websocket.connect` permission; an *unscoped* `api-key create` grants exactly that by default (empty list → `websocket.connect` only, `services/conductor/tokens.go:24-25`), so mint the fixture key **unscoped**. Only if the fixture ends up on the **OAuth** harness (where permissions are enforced — in no-auth the `noopPermissionChecker` ignores them) and you scope the key must you explicitly include `websocket.connect` (+ `application.read` for the reads). *Resolve first (spike):* whether the executor websocket accepts a `local`-org key against the **no-auth** harness or requires the **OAuth** harness (`StartOAuth`), plus host→container `ws://` networking. *Done when:* `executors`/`versions`/`metrics` return non-empty data through the CLI against the fixture. **Shared with E1** (workflow reads need executed workflows from the same running app), so build it reusable there.
