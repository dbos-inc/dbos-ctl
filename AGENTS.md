...
**D — app & API-key management (the first feature milestone).** The endpoints are unconditional, so they're exercised against the no-auth A5 harness — but with B and C in place they now also work against cloud and authenticated self-host on day one, and a minted `dbos_` key is meaningful.
