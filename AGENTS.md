...
**Identity.** The managed-vs-conductor split is gone — the generated `getCurrentUser`/`registerUser` serve both `oauth` and `managed` modes unchanged, because the managed proxy exposes conductor's shapes. The only remaining branch is `selfhosted` (no-auth), where `/v2/users*` do not exist: there `dbos whoami` reports a static `{Name: "local", OrgName: "local"}` and `register` returns a clear unsupported error. That is a mode check in the login/whoami/register commands, not a `Provider` interface with per-backend impls.
...
- [x] **A2. Root wiring** — entrypoint at `cmd/dbos/main.go`, command tree in `internal/cli`, `Use: "dbos"`, request-shaping flags (`-o/--output`, `-a/--app`, `--org`, `--url`, `--profile`; later scoped per-command via `addRequestFlags` — see Request-shaping flags), and a real `dbos version` with the ldflags/`ReadBuildInfo` resolution (see Versioning & release) plus `rootCmd.Version` for `--version`. No config file yet; `--url`/`--org` only. *Done when:* `dbos version` prints a commit from a plain `go build` and `dbos --help` is correct.
...
- [x] **C2. Auth wiring** — bearer injection, `dbos_` passthrough, refresh-on-expiry, `dbos login`/`logout`, persisting the token via B2.
- [x] **C3. Error mapping** — `problem+json` → message, 401 → "run `dbos login`", 403 + `X-DBOS-Error: past_limit` → the upgrade message; plus the exit-code contract (auth→3, not-found→4, usage→2, else 1) via an `exitError` + `SetFlagErrorFunc`. A Teams-only 403 needs no special case — conductor sends no header, and its problem+json detail already says so. (The pre-request OAuth-gate check waits for Z4, where the OAuth-only commands it guards first appear.)
- [x] **C4. Identity (whoami)** — `dbos whoami` via the generated `getCurrentUser`, with a single `selfhosted`-mode guard (static `local`). The natural end-to-end confirmation that login worked; `register` ships with the OAuth-gated org/invite flow in Z4.
- [x] **C5. Auth round-trip integration tests** — a new **OAuth-mode conductor** harness (`conductortest.StartOAuth`, `DBOS_OAUTH_ENABLED=true`) trusting an in-process **RS256 issuer** the container reaches via `host.testcontainers.internal` (testcontainers `HostAccessPorts` — no `/etc/hosts` pin, no browser, and **dependency-free**: the JWT is minted and signed with stdlib in-process, and the issuer serves its own OIDC discovery + JWKS, so no `navikt/mock-oauth2-server` and no device/password grant). One `//go:build integration` test (`TestOAuthRoundTripsIntegration`) drives both bearer sources through the CLI end to end: **(1)** a user **JWT** → `dbos whoami` (getCurrentUser); **(2)** a **`dbos_` key** minted via the `CreateToken` API (granted `application.read`, no raw SQL) → `dbos app list`. *Spike outcomes that shaped it:* conductor does **not** auto-provision (`GetUserProfile`→404 on an unknown subject), so `registerUser` is the provisioning step; and getCurrentUser is **JWT-only** (API-key callers carry an empty `sub`), so the `dbos_` path targets `app list`, not `whoami`.

**D — app & API-key management (the first feature milestone).** The endpoints are unconditional, so they're exercised against the no-auth A5 harness — but with B and C in place they now also work against a DBOS-managed Conductor and authenticated self-host on day one, and a minted `dbos_` key is meaningful.
