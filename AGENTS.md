...
- [ ] **D2. App reads** — `app get`, `app versions`, `app executors`, `app metrics` (building on A4 `app list` and D1's registered app), plus the single-object **key/value detail renderer** the 29-field `App`/`Workflow` shapes need. *Two testability layers* (see **Executor fixture** in Testing): plumbing + rendering are covered against a bare D1 app (`app get` populated; `executors`/`versions`/`metrics` render a valid empty response) plus unit tests on the renderer with synthetic structs; **populated** `executors`/`versions`/`metrics` need the conductor-connected Go Transact app — and at different strengths (`executors` survives a single connect, `versions` needs a live Healthy executor, `metrics` also needs executed workflows + an elapsed collection period). *Done when:* `app get` + the renderer round-trip against a D1 app with unit coverage, and `executors`/`versions`/`metrics` return non-empty data against the executor fixture. (`app metrics` renders `table`/`json` here; its report-shaped `csv` format lands with F1.)
