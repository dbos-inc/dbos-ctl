...
**Ordering note — the create/destroy pair leads.** App *reads* need an app to exist, and the only thing that makes one exist is `registerApp` (confirmed: `RegisterApp(org, name, {})` with an **empty body** stands up a bare app record — no executor, no deploy; C5's `dbos_`-key `app list` test relies on exactly this). So `app register`/`app delete` are the fixture primitive and come first; reads and the tuning writes build on them. Tokens are independent (a token needs only an org, which auth already supplies; `--app` scoping is optional) so they don't have the fixture problem and float to the end of D.
...
- [ ] **D2. App reads** — `app get`, `app versions`, `app executors`, `app metrics` (building on A4 `app list` and D1's registered app), plus the single-object **key/value detail renderer** (`output.Detail`/`output.Field`) the wide `App`/`Workflow` shapes need. *Two testability layers* (see **Executor fixture** in Testing): the plumbing + rendering layer against a bare D1 app, and the populated layer against the conductor-connected Go Transact app.
  - **Landed (command surface + renderer):** all four commands + `output.Detail` (omit-empty label/value, labels are the JSON keys so table and `-o json` cross-reference); `app metrics` takes `--since` (default 24h) since the endpoint requires an explicit window. Unit-tested (renderer with synthetic structs; each command against a mock server, metrics asserting the window query params). Integration `TestAppReadsBareIntegration` proves `app get` (real data through the renderer) and `app executors` (DB-backed → empty-but-valid on a bare app) against the no-auth harness.
  - **Remaining (populated reads via the executor fixture):** `versions` dispatches to a live Healthy executor (a bare app returns `ErrNoHealthyExecutors` — *not* an empty list), `executors` populated needs one connect, `metrics` needs a live executor that ran workflows + an elapsed collection period. All three need the Go Transact app fixture, shared with E1. (`app metrics` renders `table`/`json` here; its report-shaped `csv` format lands with F1.)
- [ ] **D3. App writes** — `app set-version`, `app update` (the self-hosted tuning surface: `executorTimeoutSecs`, `gcRowsThreshold`, `gcTimeThresholdMs`, `globalTimeoutMs`, `privateMode`). *Done when:* an update round-trips through `app get`.
- [ ] **D4. Tokens** — `token list|create|delete`; `create` prints the bare `dbos_` secret once, scoped by `--app`/`--permission`, and is `-o ids` friendly. *Done when:* a token minted against the harness can list and delete itself. This is the login-once-then-mint-a-scoped-key CI flow — cross-mode from the start now that auth (C) is already in place. Independent of the app CRUD above, so it can move earlier if a fully self-contained feature is wanted first.

**E — workflows, queues, schedules.** Small PRs over the A/B/C/D plumbing.

- [ ] **E1. Workflow reads** — `list` (search-backed, full filter set), `get`, `steps`, `events` (reuses D2's detail renderer). Depends on the **Executor fixture** (Testing): workflow reads need real executed workflows, so the conductor-connected Go Transact app is mandatory here, not merely enriching.
