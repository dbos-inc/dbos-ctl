...
- [ ] **C5. Auth round-trip integration tests** — a new **OAuth-mode conductor** harness variant (`DBOS_OAUTH_ENABLED=true`), plus two `//go:build integration` round-trips that prove an authenticated `app list` actually works against it: (1) a **`dbos_` key** DB-seeded straight into the `tokens` table (stored plaintext, so no hashing) → `DBOS_TOKEN=dbos_…`; (2) a **JWT** minted by a lightweight `navikt/mock-oauth2-server` (custom `aud`/`sub`, its JWKS trusted by conductor) → bearer. Together they cover the two bearer sources end to end — the piece the device-flow tests can't reach (browser approval). *First step:* a spike answering whether conductor auto-provisions a user/org from the caller identity in OAuth mode, or requires a seeded user — it gates both round-trips. (This is the tier-4 work the plan flagged as deferred; the KC_HOSTNAME-style issuer pin + minting the token via the mapped port makes it a host `go test`, no `/etc/hosts` or browser.)
