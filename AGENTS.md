...
- [ ] **E1. Workflow reads** — `list` (search-backed, full filter set), `get`, `steps`, `events` (reuses D2's detail renderer). Depends on **D4's executor fixture**: workflow reads need real executed workflows, so the conductor-connected Go Transact app is mandatory here, not merely enriching.
...
- [ ] **F8. Interactive app/permission selection (optional)** — for commands that take apps or permissions, let the user pick them from a live list instead of typing exact strings: `api-key create`'s `--app`/`--permission` first, and any later command with an app or permission argument. **Opt-in, never automatic** — triggered by an explicit flag (e.g. `--interactive`/`-i`), not silently on a TTY, so scripts and the plain flag-driven path are untouched. Reuses the `isInteractive`/`confirm` infra from `app delete`; the permission catalog comes from `listPermissions` (needs it always-enabled — see the API-keys "Upcoming" note), the app list from `listApps`.
